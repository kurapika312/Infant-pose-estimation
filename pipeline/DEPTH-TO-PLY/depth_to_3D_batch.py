...
def getCameraIntrinsics(path: pathlib.Path)->tuple:
    intrinsics: np.ndarray = np.load(f'{path.resolve()}')
    fx, fy = intrinsics[0, 0], intrinsics[1, 1]
    cx, cy = intrinsics[0, 2].astype(float), intrinsics[1, 2].astype(float)
    return fx, fy, cx, cy
...
def batchDepthToPLY(
    ...
    for i, d_png in tqdm.tqdm(enumerate(glob_result), dynamic_ncols=True, total=len(glob_result)):
        ...
        output_path = output_folder.joinpath(f'{d_png.stem}{OUTPUT_TYPE}')
        ...
        if(save_pca_cloud):
            ...
            output_path_pca = output_pca_folder.joinpath(f'{d_png.stem}{OUTPUT_TYPE}')
